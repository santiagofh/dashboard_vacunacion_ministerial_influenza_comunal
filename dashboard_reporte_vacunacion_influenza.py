...
#%%
# Función para leer el último archivo creado con el nombre Reporte_vacunas_ministeriales en la carpeta "Reporte"
def leer_ultimo_reporte_vacunas_ministeriales(directorio):
    # Buscar todos los archivos en el directorio que contengan "Reporte_vacunas_ministeriales"
    archivos = [os.path.join(directorio, f) for f in os.listdir(directorio) if "Reporte_vacunas_ministeriales" in f and f.endswith(".csv")]
...
# Leer el último reporte de vacunas ministeriales
ultimo_reporte, fecha_creacion = leer_ultimo_reporte_vacunas_ministeriales(directorio_reporte)

# Mostrar el DataFrame en Streamlit si se encuentra un archivo
if ultimo_reporte is not None:
    st.title("Reporte de Vacunaciones Ministeriales")
    st.write(f"Último Reporte de Vacunas Ministeriales (Creado el {fecha_creacion.strftime('%Y-%m-%d')}):")
    ...
    st.write("No se encontró ningún reporte de vacunas ministeriales.")
